...
function LoginForm({ onLogin }: { onLogin: (token: string) => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError("");
    try {
      const res = await fetch("/api/ai-for-all/admin/auth", {
        method: "POST",
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center px-4">
      ...
      <div className="relative z-10 w-full max-w-sm">
        ...
        <div className="text-center mt-4">
          <Link href="/apps/ai-for-all" className="text-xs text-muted hover:text-foreground transition-colors flex items-center justify-center gap-1">
            <ArrowLeft className="h-3 w-3" /> Back to AI for All
          </Link>
        </div>
      </div>
    </div>
      ...
      <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-xl border-b border-border/40">
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href="/apps/ai-for-all" className="text-muted hover:text-foreground transition-colors">
              <ArrowLeft className="h-4 w-4" />
            </Link>
            <span className="font-bold text-sm">AI for All — Admin</span>
          </div>
          <button
            onClick={logout}
